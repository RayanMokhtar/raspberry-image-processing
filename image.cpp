...
public:
    ...
    // Fonction Canny sans multithreading - version séquentielle simple
    Mat cannySimple(cv::Mat& input, int seuil = 50) {
        ...
        // Appliquer Canny directement sur toute l'image
        Canny(input, output, threshold1, threshold2, apertureSize);
        
        return output;
    }
    
    // Fonction Sobel sans multithreading - version séquentielle standard
    Mat sobelSimple(Mat& input, int threshold = 50) {
        // Création de matrices temporaires
        Mat grad_x, grad_y;
        Mat abs_grad_x, abs_grad_y;
        Mat gradient, output;
        
        // Calcul des gradients dans les directions x et y sans multithreading
        ...
        cv::threshold(gradient, output, threshold, 255, THRESH_BINARY);
    ...
    // Fonction Hough sans multithreading - version séquentielle simple
    int houghSimple(Mat& edges, Mat& result, bool& is_straight, bool is_souple=false) {
        // Paramètres de détection
        int threshold = is_souple ? 15 : 30;
        int minLineLength = is_souple ? 20 : 30;
        int maxLineGap = is_souple ? 20 : 15;
        
        // Détection des lignes sans multithreading
        vector<Vec4i> lines;
        ...
        for(const auto& l : lines) {
            ...
            int x1 = l[0], x2 = l[2];
            int x_center = (x1 + x2) / 2;
            bool is_center = abs(x_center - img_center) < img_center * 0.45;
            bool is_vertical = (abs(angle) < 25 || abs(angle) > 155);
    ...
    Mat pretraitementImage(const string& imagePath) {
        ...
        // Application du filtre de Sobel/Canny sur l'image COMPLÈTE (sans ROI)
        auto startSobel = high_resolution_clock::now();
        Mat edges = cannySimple(gray, 25);
        bitwise_and(edges, colorMask, edges); // Appliquer le masque de couleur sur l'image complète
        auto endSobel = high_resolution_clock::now();
        metrics.sobelTime = duration_cast<milliseconds>(endSobel - startSobel).count();
        
        // Application de la transformée de Hough sans multithreading sur l'image complète
        auto startHough = high_resolution_clock::now();
        Mat result = input.clone();
        bool is_straight = false;
        int numLines = houghSimple(edges, result, is_straight, true);
        
        auto startModeSouple = high_resolution_clock::now();
        // Si peu de lignes détectées, on essaie avec des paramètres plus souples
        if(numLines < nombreMinimalLignes) {
            // Traitement avec paramètres plus souples sur toute l'image
            edges = cannySimple(gray, 18); // Seuil plus bas
            bitwise_and(edges, colorMask, edges);
            result = input.clone();
            is_straight = false;
            numLines = houghSimple(edges, result, is_straight, true); 
        }
