...
Mat extractStaticROI(const Mat& inputImage) {
    // Calcul des dimensions de la ROI
    int top = static_cast<int>(inputImage.rows * 0.25);    // 25% du haut
    int left = static_cast<int>(inputImage.cols * 0.10);   // 10% de la gauche
    int right = static_cast<int>(inputImage.cols * 0.10);  // 10% de la droite
    
    // Calcul de la largeur et hauteur de la ROI
    int roiWidth = inputImage.cols - left - right;
    int roiHeight = inputImage.rows - top;
    
    // Création du rectangle de la ROI
    Rect roiRect(left, top, roiWidth, roiHeight);
    
    // Extraction de la ROI
    return inputImage(roiRect);
}
...
public:
    ...
    Mat applySobel(Mat& input, int threshold = 60) {
        ...
        Mat binary;
        cv::threshold(output, binary, 0, 255, THRESH_BINARY);
        return binary; // Retourner l'image binarisée au lieu de output
    }
    ...
    int applyHoughLinesP_adaptatif(Mat& edges, Mat& result, int roi_y, int roi_mode, bool& is_straight, bool permissif=false) {
        ...
        int threshold = permissif ? 15 : 25;
        int minLineLength = permissif ? 20 : 50;
        int maxLineGap = permissif ? 40 : 10;
